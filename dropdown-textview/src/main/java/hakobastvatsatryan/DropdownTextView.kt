...
class DropdownTextView : LinearLayout {
	...
	private fun readAttributes(attrs: AttributeSet) {
		val themeAttributes = context.theme.obtainStyledAttributes(
				attrs,
		...
		themeAttributes.getValue(R.styleable.DropdownTextView_title_text, typeValue)
		...
		themeAttributes.getValue(R.styleable.DropdownTextView_ddTextviewTitleTextColor, typeValue)
		titleTextColor = when (typeValue.type) {
			TypedValue.TYPE_REFERENCE -> ContextCompat.getColor(context, typeValue.resourceId)
			else -> typeValue.data
		}

		themeAttributes.getValue(R.styleable.DropdownTextView_title_text_color_expanded, typeValue)
		titleTextColorExpanded = when (typeValue.type) {
			TypedValue.TYPE_REFERENCE -> ContextCompat.getColor(context, typeValue.resourceId)
			else -> typeValue.data
		}

		titleTextSizeRes = themeAttributes.getResourceId(R.styleable.DropdownTextView_titleTextSize, -1)
		titleFontRes = themeAttributes.getResourceId(R.styleable.DropdownTextView_title_font, -1)

		themeAttributes.getValue(R.styleable.DropdownTextView_content_text, typeValue)
		...
		themeAttributes.getValue(R.styleable.DropdownTextView_contentTextColor, typeValue)
		contentTextColor = when (typeValue.type) {
			TypedValue.TYPE_REFERENCE -> ContextCompat.getColor(context, typeValue.resourceId)
			else -> typeValue.data
		}

		contentTextSizeRes = themeAttributes.getResourceId(R.styleable.DropdownTextView_contentTextSize, -1)
		contentFontRes = themeAttributes.getResourceId(R.styleable.DropdownTextView_content_font, -1)

		bgRegularDrawableRes = themeAttributes.getResourceId(R.styleable.DropdownTextView_bg_drawable_regular, -1)
		bgExpandedDrawableRes = themeAttributes.getResourceId(
				R.styleable.DropdownTextView_bg_drawable_expanded,
				-1
		)

		panelPaddingRes = themeAttributes.getResourceId(
				R.styleable.DropdownTextView_panelPadding,
				R.dimen.panel_default_padding
		)
		contentPaddingRes = themeAttributes.getResourceId(
				R.styleable.DropdownTextView_contentPadding,
				R.dimen.content_default_padding
		)

		arrowDrawableRes = themeAttributes.getResourceId(
				R.styleable.DropdownTextView_arrowDrawable,
				R.drawable.ic_arrow
		)

		expandDuration = themeAttributes.getInteger(R.styleable.DropdownTextView_expand_duration, 300)
	}
